                    ...
                    <div class="card border-0 shadow-sm rounded-lg overflow-hidden mb-4">
                        ...
                        <div class="card-body p-4">
                            <div class="mb-4">
                                ...
                                <small class="text-muted">Posté
                                    le {{ $consultation->created_at->format('d/m/Y H:i') }}</small>
                            </div>
                            ...
                            @if($consultation->admin_response)
                                <div class="mt-4">
                                    <h6 class="font-weight-bold text-orange small text-uppercase"><i
                                            class="fa fa-comment-dots mr-2"></i>Réponse de la COOPROM :</h6>
                                    <div class="p-4 border rounded shadow-sm bg-white"
                                         style="border-left: 5px solid #fa584d !important;">
                                        {!! nl2br(e($consultation->admin_response)) !!}
                                        <div class="mt-3 pt-3 border-top text-muted small">
                                            Par un expert juridique,
                                            le {{ $consultation->answered_at ? $consultation->answered_at->format('d/m/Y H:i') : $consultation->updated_at->format('d/m/Y H:i') }}
                                        </div>
                                    </div>
                                </div>
                            @else
                                <div class="mt-4 p-4 text-center bg-light rounded">
                                    ...
                                    <p class="mb-0 text-muted">Votre demande est en cours de traitement par notre équipe
                                        juridique. Vous recevrez une notification dès qu'une réponse sera apportée.</p>
                                </div>
                            @endif
                        </div>
                    </div>
