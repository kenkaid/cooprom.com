...
@section('content')
    ...
    @php
        ...
        $breadcumb_table = ['Poser une question'];
    @endphp
                            ...
                            <form action="{{ route('member.legal.store') }}" method="POST">
                                ...
                                <div class="form-group mb-4">
                                    ...
                                    <input type="text" name="subject"
                                           ...
                                           value="{{ old('subject') }}"
                                           placeholder="Ex: Litige sur un contrat de production" required>
                                    @error('subject')
                                ...
                                <div class="form-group mb-4">
                                    ...
                                    <select name="category" class="form-control @error('category') is-invalid @enderror"
                                        ...
                                        <option
                                            value="copyright" {{ old('category') == 'copyright' ? 'selected' : '' }}>
                                            Droit d'auteur
                                        </option>
                                        <option value="contract" {{ old('category') == 'contract' ? 'selected' : '' }}>
                                            Contrats
                                        </option>
                                        <option value="social" {{ old('category') == 'social' ? 'selected' : '' }}>
                                            Social / Statut d'artiste
                                        </option>
                                        <option value="tax" {{ old('category') == 'tax' ? 'selected' : '' }}>Fiscalité
                                        </option>
                                        <option value="other" {{ old('category') == 'other' ? 'selected' : '' }}>Autre
                                        </option>
                                    </select>
                                    @error('category')
                                    <div class="invalid-feedback">{{ $message }}</div> @enderror
                                ...
                                <div class="form-group mb-4">
                                    ...
                                    <textarea name="message" class="form-control @error('message') is-invalid @enderror"
                                              rows="8"
                                              placeholder="Expliquez ici votre situation de manière détaillée..."
                                ...
                                <div class="text-right">
                                    <a href="{{ route('member.legal.index') }}"
                                       class="btn btn-light rounded-pill px-4 mr-2">Annuler</a>
                                    <button type="submit"
                                            class="btn btn_orange text-white rounded-pill px-5 py-2 font-weight-bold">
                                        Envoyer ma demande
                                    </button>
                                </div>
                            </form>
